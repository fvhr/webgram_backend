...
class TierRepositoryProvider(Provider):
    ...
    async def get_tier_repository(self, session_maker: async_sessionmaker[AsyncSession], db_mapper: TierDBMapper) \
            -> TierRepositoryProtocol:
        return TierRepositorySQLAlchemy(session_maker=session_maker, mapper=db_mapper)

    @provide(scope=Scope.REQUEST)
    async def get_view_tier_repository(self, session_maker: async_sessionmaker[AsyncSession], db_mapper: TierDBMapper) \
            -> ViewTierRepositoryProtocol:
        return ViewTierRepositorySQLAlchemy(session_maker=session_maker, mapper=db_mapper)
