...
class NumberRepositorySQLAlchemy(NumbersRepositoryProtocol):
    ...
    async def create_number(self, number: Numbers) -> Numbers | None:
        try:
            async with self.session_maker() as session:
                stmt = select(NumberModel).where(NumberModel.number_uuid == number.number_uuid)
                result = await session.execute(stmt)
                number_model = result.scalar_one_or_none()
                if number_model:
                    return None
                number_model = self.mapper.to_model(number)
                session.add(number_model)
                await session.commit()
                return number
        except IntegrityError as e:
            logger.critical(f"Conflict while saving number '{number.number_uuid}': {e}")
            raise ConflictRepositoryError(f"Conflict while saving number '{number.number_uuid}': {e}") from e
        except SQLAlchemyError as e:
            logger.critical(f"Failed to save number '{number.number_uuid}': {e}")
            raise RepositoryError(f"Failed to save number '{number.number_uuid}': {e}") from e
    ...
    async def delete_number(self, number_uuid: str) -> None:
        try:
            async with self.session_maker() as session:
                stmt = delete(NumberModel).where(NumberModel.number_uuid == number_uuid)
                await session.execute(stmt)
                await session.commit()
        except SQLAlchemyError as e:
            logger.critical(f"Failed to delete number '{number_uuid}': {e}")
            raise RepositoryError(f"Failed to delete number '{number_uuid}': {e}") from e