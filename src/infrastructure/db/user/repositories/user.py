...
class UserRepositorySQLAlchemy(UserRepositoryProtocol):
    ...
    async def create_user(self, user: User) -> UUID | None:
        try:
            async with self.session_maker() as session:
                stmt = select(UserModel).where(UserModel.user_uuid == user.user_uuid)
            ...
            logger.critical(f"Conflict while saving user '{user.user_uuid}': {e}")
            raise ConflictRepositoryError(f"Conflict while saving user '{user.user_uuid}': {e}") from e
        except SQLAlchemyError as e:
            logger.critical(f"Failed to save user '{user.user_uuid}': {e}")
            raise RepositoryError(f"Failed to save user '{user.user_uuid}': {e}") from e

    async def delete_user(self, user_uuid: str) -> None:
        try:
            async with self.session_maker() as session:
                stmt = delete(UserModel).where(UserModel.user_uuid == user_uuid)
                await session.execute(stmt)
                await session.commit()
        except SQLAlchemyError as e:
            logger.critical(f"Failed to delete user '{user_uuid}': {e}")
            raise RepositoryError(f"Failed to delete user '{user_uuid}': {e}") from e
    ...
    async def update_user(self, user: UpdateUserDTO) -> UUID | None:
        try:
            ...
            logger.critical(f"Conflict while update user '{user.user_uuid}': {e}")
            raise ConflictRepositoryError(f"Conflict while update user '{user.user_uuid}': {e}") from e
        except SQLAlchemyError as e:
            logger.critical(f"Failed to update user '{user.user_uuid}': {e}")
            raise RepositoryError(f"Failed to update user '{user.user_uuid}': {e}") from e

    async def change_password(self, user_uuid: str, new_password: str) -> UUID | None:
        try:
            async with self.session_maker() as session:
                stmt = select(UserModel).where(UserModel.user_uuid == user_uuid)
                result = await session.execute(stmt)
                model = result.scalar_one_or_none()
                if not model:
                    return None
            ...
            logger.critical(f"Conflict while update password user '{user_uuid}': {e}")
            raise ConflictRepositoryError(f"Conflict while update password user '{user_uuid}': {e}") from e
        except SQLAlchemyError as e:
            logger.critical(f"Failed to update password user '{user_uuid}': {e}")
            raise RepositoryError(f"Failed to update password user '{user_uuid}': {e}") from e