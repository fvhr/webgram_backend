...
class RoleRepositorySQLAlchemy(RoleRepositoryProtocol):
    ...
    async def get_role(self, role_uuid: str) -> Role | None:
        try:
            ...
            raise RepositoryError(f"Failed to retrieve role by role_uuid '{role_uuid}': {e}") from e
    ...
    async def get_role_by_role_name(self, role_name: str) -> Role | None:
        try:
            ...
            raise RepositoryError(f"Failed to retrieve role by role_name '{role_name}': {e}") from e
    ...
    async def get_roles(self) -> list[Role]:
        try:
            ...
            raise RepositoryError(f'Failed to retrieve role: {e}') from e

    async def create_role(self, role: Role) -> Role | None:
        try:
            async with self.session_maker() as session:
                stmt = select(RoleModel).where(RoleModel.role_uuid == role.role_uuid)
                result = await session.execute(stmt)
            ...
            raise ConflictRepositoryError(f"Conflict while saving role '{role.role_uuid}': {e}") from e
        except SQLAlchemyError as e:
            logger.critical(f"Failed to save role '{role.role_uuid}': {e}")
            raise RepositoryError(f"Failed to save role '{role.role_uuid}': {e}") from e
    ...
    async def delete_role(self, role_uuid: str) -> None:
        try:
            ...
            raise RepositoryError(f"Failed to delete role '{role_uuid}': {e}") from e